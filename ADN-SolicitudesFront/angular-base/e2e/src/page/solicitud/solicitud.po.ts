...
export class SolicitudPage {
  ...
  private inputClienteCelularContacto = element(
    by.id('clienteCelularContacto')
  );
  ...
  private botonActualizadoVentanaModal = element(
    by.buttonText('Si, actualizar')
  );
  private textoSweetAlert = element(by.id('swal2-title'));
  private botonEditarPrimerSolicitud = element
    .all(by.css('.btn-editar-solicitud'))
    .first();
  ...
  async ingresarClienteCelularContacto(inputClienteCelularContacto) {
    await this.inputClienteCelularContacto.sendKeys(
      inputClienteCelularContacto
    );
  }
